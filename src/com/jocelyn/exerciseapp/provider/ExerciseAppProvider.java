...
public class ExerciseAppProvider extends ContentProvider {
	...
	static {
		...
		// Workouts
		sURIMatcher.addURI(ExerciseAppManager.getAuthority(),
				ExerciseAppManager.getWorkoutsPath(), WORKOUTS);
		sURIMatcher.addURI(ExerciseAppManager.getAuthority(),
				ExerciseAppManager.getWorkoutsPath() + "/#", WORKOUT_ID);
		// gets all Exercises of a specific Workout
		sURIMatcher.addURI(ExerciseAppManager.getAuthority(),
				ExerciseAppManager.getWorkoutsPath() + "/#/"
						+ ExerciseAppManager.getExercisesPath(),
				WR_ID_EXERCISES);

		// Exercises
		sURIMatcher.addURI(ExerciseAppManager.getAuthority(),
				ExerciseAppManager.getExercisesPath(), EXERCISES);
		sURIMatcher.addURI(ExerciseAppManager.getAuthority(),
				ExerciseAppManager.getExercisesPath() + "/#", EXERCISE_ID);
		// select distinct exercises
		sURIMatcher.addURI(ExerciseAppManager.getAuthority(),
				ExerciseAppManager.getExercisesPath() + "/distinct",
				DISTINCT_EXERCISE);

		// WRE
		// gets all WRE of a specific Workout
		sURIMatcher.addURI(ExerciseAppManager.getAuthority(),
				ExerciseAppManager.getWrExercisesPath() + "/"
						+ ExerciseAppManager.getWorkoutsPath() + "/#",
				WRE_WORKOUT_ID);
		sURIMatcher.addURI(ExerciseAppManager.getAuthority(),
				ExerciseAppManager.getWrExercisesPath(), WR_EXERCISES);
		sURIMatcher.addURI(ExerciseAppManager.getAuthority(),
				ExerciseAppManager.getWrExercisesPath() + "/#", WR_EXERCISE_ID);

		// Records
		sURIMatcher.addURI(ExerciseAppManager.getAuthority(),
				ExerciseAppManager.getRecordsPath(), RECORDS);
		// gets all Exercises of a specific Workout
		sURIMatcher.addURI(ExerciseAppManager.getAuthority(),
				ExerciseAppManager.getWorkoutsPath() + "/#/"
						+ ExerciseAppManager.getExercisesPath() + "/"
						+ ExerciseAppManager.getRecordsPath(),
				WR_ID_EXERCISES_RECORDS);
	...
	public String getType(Uri uri) {
		...
		case WR_EXERCISES:
			return WRExercises.CONTENT_TYPE; // need to think about uri for
												// joining tables*/
		case WR_EXERCISE_ID:
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		case WORKOUTS:
			rowsAffected = sqlDB.delete(
					WorkoutRoutineTable.TABLE_WORKOUTROUTINE, selection,
					selectionArgs);
			break;
		case WORKOUT_ID:
			...
			if (TextUtils.isEmpty(selection)) {
				...
				rowsAffected = sqlDB.delete(
						WorkoutRoutineTable.TABLE_WORKOUTROUTINE, selection
								+ " and " + WorkoutRoutineTable.COLUMN_ID + "="
								+ id, selectionArgs);
			}
			break;
		/*
		 * case WR_EXERCISES: rowsAffected =
		 * sqlDB.delete(WorkoutRoutineExerciseTable
		 * .TABLE_WORKOUTROUTINE_EXERCISE, selection, selectionArgs); break;
		 */
		case WR_EXERCISE_ID:
			id = uri.getLastPathSegment();
			if (TextUtils.isEmpty(selection)) {
				rowsAffected = sqlDB
						.delete(WorkoutRoutineExerciseTable.TABLE_WORKOUTROUTINE_EXERCISE,
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		case WORKOUTS:
			rowId = mDB.getWritableDatabase().insert(
					WorkoutRoutineTable.TABLE_WORKOUTROUTINE, "", values);
			if (rowId > 0) {
		...
		case RECORDS:
			rowId = mDB.getWritableDatabase().insert(RecordTable.TABLE_RECORD,
					"", values);
			if (rowId > 0) {
	...
	public Cursor query(Uri uri, String[] projection, String selection,
		...
		case WORKOUT_ID:
			qb.setTables(WorkoutRoutineTable.TABLE_WORKOUTROUTINE);
			qb.appendWhere(WorkoutRoutineTable.COLUMN_ID + "="
		...
		case WR_EXERCISE_ID:
			...
			qb.appendWhere(WRExercisesColumns.COLUMN_EXERCISE_ID + " = "
					+ ExercisesColumns.COLUMN_ID + " AND "
		...
		case WR_ID_EXERCISES:
			...
			qb.appendWhere(WorkoutsColumns.COLUMN_ID + " = "
					+ WRExercisesColumns.COLUMN_WORKOUT_ID + " AND "
			...
			break;
		// does it make a difference if search from workout table or wrexercise
		// table?
		case WR_ID_EXERCISES_RECORDS:
			qb.setTables(WorkoutRoutineExerciseTable.TABLE_WORKOUTROUTINE_EXERCISE
					+ " INNER JOIN " + WorkoutRoutineTable.TABLE_WORKOUTROUTINE
					+ " ON (" + WorkoutsColumns.COLUMN_ID + " = " + WRExercisesColumns.COLUMN_WORKOUT_ID + ") "
					+ " INNER JOIN " + ExerciseTable.TABLE_EXERCISE
					+ " ON (" + ExercisesColumns.COLUMN_ID + " = " + WRExercisesColumns.COLUMN_EXERCISE_ID + ")"
					+ " LEFT OUTER JOIN " + RecordTable.TABLE_RECORD
					+ " ON (" + RecordsColumns.COLUMN_WRKT_RTNE_E_ID + " = " + WRExercisesColumns.COLUMN_ID + ") ");
			break;
		case EXERCISE_ID:
			qb.setTables(ExerciseTable.TABLE_EXERCISE);
			qb.appendWhere(ExerciseTable.COLUMN_ID + "="
					+ uri.getLastPathSegment());
			break;
		case EXERCISES:
			qb.setTables(ExerciseTable.TABLE_EXERCISE);
		 ...
		 * case WR_EXERCISE_ID:
		 * qb.setTables(WorkoutRoutineExerciseTable.TABLE_WORKOUTROUTINE_EXERCISE
		 * ); qb.appendWhere(WorkoutRoutineExerciseTable.COLUMN_ID + "=" +
		 * uri.getLastPathSegment());
		 * 
		 * break; case WR_EXERCISES:
		 * qb.setTables(WorkoutRoutineExerciseTable.TABLE_WORKOUTROUTINE_EXERCISE
		 * ); break;
		 */
		default:
		...
		Cursor cursor = qb.query(mDB.getReadableDatabase(), projection,
